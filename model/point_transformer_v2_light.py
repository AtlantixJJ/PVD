...
class PointBatchNormCond(nn.Module):
    """
    Batch Normalization for Point Clouds data in shape of [B*N, C], [B*N, L, C]
    """

    def __init__(self, embed_channels, cond_channels):
        super().__init__()
        self.norm = nn.BatchNorm1d(embed_channels, affine=False)
        self.affine = torch.nn.Linear(cond_channels, embed_channels * 2, bias=False)

    def forward(self, input: torch.Tensor, cond: torch.Tensor, offset: torch.Tensor) -> torch.Tensor:
        scale, shift = torch.chunk(self.affine(cond), 2, 1)
        indice = offset2batch(offset)
        scale = (scale + 1)[indice]
        shift = shift[indice]
        
        if input.dim() == 3:
            out = self.norm(input.transpose(1, 2).contiguous())
            out = out * scale.unsqueeze(1) + shift.unsqueeze(1)
            return out.transpose(1, 2).contiguous()
        elif input.dim() == 2:
            if input.shape[0] == 1: # sometimes there is only one global representation
                return input * scale + shift
            return self.norm(input) * scale + shift
        else:
            raise NotImplementedError
...
class BlockCond(nn.Module):
    def __init__(self,
        ...
        self.fc1 = nn.Linear(embed_channels, embed_channels, bias=False)
        self.fc3 = nn.Linear(embed_channels, embed_channels, bias=False)
        self.norm1 = PointBatchNormCond(embed_channels, cond_channels)
        self.norm2 = PointBatchNormCond(embed_channels, cond_channels)
        self.norm3 = PointBatchNormCond(embed_channels, cond_channels)
        self.act = nn.ReLU(inplace=True)
        self.enable_checkpoint = enable_checkpoint
        self.drop_path = DropPath(drop_path_rate) if drop_path_rate > 0. else nn.Identity()

    def forward(self, points, reference_index, cond):
        coord, feat, offset = points
        identity = feat
        feat = self.act(self.norm1(self.fc1(feat), cond, offset))
        feat = self.attn(feat, coord, reference_index) \
            if not self.enable_checkpoint else checkpoint(self.attn, feat, coord, reference_index)
        feat = self.act(self.norm2(feat, cond, offset))
        feat = self.norm3(self.fc3(feat), cond, offset)
        feat = identity + self.drop_path(feat)
        feat = self.act(feat)
        return [coord, feat, offset]
...
class BlockSequence(nn.Module):
    def __init__(self,
        ...
        self.has_cond = cond_channels is not None
        kwargs = dict(
                embed_channels=embed_channels,
                groups=groups,
                qkv_bias=qkv_bias,
        ...
        for i in range(depth):
            if self.has_cond:
                block = BlockCond(
                    ...
                    cond_channels=cond_channels, **kwargs)
            else:
                block = Block(
                    drop_path_rate=drop_path_rates[i], **kwargs)
            self.blocks.append(block)

    def forward(self, points, cond=None):
        coord, feat, offset = points
        ...
        for block in self.blocks:
            if self.has_cond:
                points = block(points, reference_index, cond)
            else:
...
class Encoder(nn.Module):
    ...
    def forward(self, points, cond=None):
        ...
        if self.has_cond:
            return self.blocks(points, cond), cluster
        else:
...
class Decoder(nn.Module):
    ...
    def forward(self, points, skip_points, cluster, cond=None):
        points = self.up(points, skip_points, cluster)
        if self.has_cond:
            return self.blocks(points, cond)
        else:
...
class PointTransformerCond(nn.Module):
    def __init__(self,
                 input_channels,
                 output_channels,
                 patch_embed_depth=1,
                 ...
                 enc_channels=(96, 192, 384, 512),
                 enc_groups=(12, 24, 48, 64),
                 enc_neighbours=(8, 8, 8, 8), #(16, 16, 16, 16),
                 dec_depths=(1, 1, 1, 1),
                 dec_channels=(48, 96, 192, 384),
                 dec_groups=(6, 12, 24, 48),
                 dec_neighbours=(8, 8, 8, 8), #(16, 16, 16, 16),
                 grid_sizes=(0.06, 0.12, 0.24, 0.48),
                 attn_qkv_bias=True,
                 pe_multiplier=False,
                 pe_bias=True,
        ...
        super(PointTransformerCond, self).__init__()
        ...
        self.time_embed = MLP(
            ...
            dtype=torch.float32,
            width=cond_channels,
            init_scale=math.sqrt(1.0 / cond_channels))
        ...
        self.seg_head = nn.Sequential(
            ...
            PointBatchNorm(dec_channels[0]),
            nn.ReLU(inplace=True),
            nn.Linear(dec_channels[0], output_channels)
        ) if output_channels > 0 else nn.Identity()

    def forward(self, data: torch.Tensor, timestep: torch.Tensor):
        """
        Args:
            data: (B, D, N) point cloud
        """
        B, D, N = data.shape
        ...
        # a batch of point cloud is a list of coord, feat and offset
        points = [coord, data, offset]
        points = self.patch_embed(points)
        skips = [[points]]
        for i in range(self.num_stages):
            points, cluster = self.enc_stages[i](points, t_embed)
            skips[-1].append(cluster)  # record grid cluster of pooling
            skips.append([points])  # record points info of current stage
        ...
        for i in reversed(range(self.num_stages)):
            skip_points, cluster = skips.pop(-1)
            points = self.dec_stages[i](points, skip_points, cluster, t_embed)
        coord, feat, offset = points
        seg_logits = self.seg_head(feat)
        return seg_logits.view(B, N, D).permute(0, 2, 1)