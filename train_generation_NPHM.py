...
from model.point_transformer_v2_light import PointTransformerCond
from datasets.nphm import ScannerData
...
class GaussianDiffusion:
    ...
    def p_losses(self, denoise_fn, data_start, t, noise=None):
        ...
        #print(noise.shape, data_start.shape, noise.dtype, data_start.dtype)
    ...
    def __init__(self, args, betas, loss_type: str, model_mean_type: str, model_var_type:str):
        ...
        if args.arch == "PVCNN":
            self.model = PVCNN2(
                ...
                extra_feature_channels=args.nc - 3)
        elif args.arch == "VPT":
            self.model = PointDiffusionTransformer(
                device=torch.device("cuda"), dtype=torch.float32,
                input_channels=args.nc,
                output_channels=args.nc,
                n_ctx=args.npoints)
        elif args.arch == "PT":
            self.model = PointTransformerCond(
                input_channels=args.nc,
                output_channels=args.nc)
            self.model.to("cuda")
...
def get_dataset(dataroot, npoints, category):
    tr_dataset = ScannerData(
        mode='train',
        n_supervision_points_face=npoints,
        n_supervision_points_non_face=0,
        batch_size=16)
    te_dataset = ScannerData(
        mode='val',
        n_supervision_points_face=npoints,
        n_supervision_points_non_face=0,
        batch_size=16)
...
def train(gpu, opt, output_dir, noises_init):
    ...
    ''' data '''
    # the path is included in 
    train_dataset, _ = get_dataset("", opt.npoints, opt.category)
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        for i, data in enumerate(dataloader):
            #order = 2 ** int(torch.randint(-2, 3, (1,))[0])
            #dataloader.dataset.set_n_points(int(order * opt.npoints))

            x = data['train_points'].transpose(1,2)
            #noises_batch = noises_init[data['idx'].squeeze()].transpose(1,2)
...
def parse_args():
    ...
    parser.add_argument('--nc', default=9, type=int)
    parser.add_argument('--npoints', default=1024, type=int)
    '''model'''
    parser.add_argument('--arch', default="PT", help="PT | PVCNN")
