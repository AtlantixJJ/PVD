...
class Model(nn.Module):
    ...
    def gen_samples(self, shape, device, noise_fn=torch.randn, constrain_fn=lambda x, t:x,
                    ...
                    keep_running=False):
        return self.diffusion.p_sample_loop(self._denoise,
                    shape=shape, device=device, noise_fn=noise_fn,
...
def generate(model, opt):
    for i in range(opt.n_repeat):
        gen = model.gen_samples(
            torch.Size([1, opt.nc, opt.npoints]), 'cuda', clip_denoised=False)
        gen = gen.detach().cpu().permute(0, 2, 1) # [1, N, 9]
        colors = ((gen[0, :, 6:9].clamp(-1, 1) + 1) * 127.5).numpy().astype("uint8")
        path = f"{opt.eval_path}/{opt.npoints}_r{i}.ply"
        print(gen.shape)
        trimesh.PointCloud(gen[0, :, :3], colors).export(path)
...
def parse_args():
    ...
    parser.add_argument('--nc', default=9)
